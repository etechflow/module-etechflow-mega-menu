 ...
 *   plan           : plan slug (e.g. mm_starter)
 *   settings_url   : link back to Stores → Config → eTechFlow → Mega Menu
 *   management_url : link back to the gate
...
class Activated extends Template
{
    ...
    public function getSettingsUrl(): string
    {
        ...
        if ($url === '') {
            $url = (string) $this->getUrl('adminhtml/system_config/edit', ['section' => 'etechflow_megamenu']);
        }
        return $url;
    }
    ...
    public function getManagementUrl(): string
    {
        ...
        if ($url === '') {
            $url = (string) $this->getUrl('etechflow_megamenu/license/gate');
        }
        return $url;
    }
}
