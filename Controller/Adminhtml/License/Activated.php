 ...
 * Called as: /admin/etechflow_megamenu/license/activated?session_id=cs_xxx&plan=...&domain=...
 *
...
class Activated extends Action
{
    ...
    public function execute(): ResultInterface
    {
        ...
        if ($sessionId === '' || !preg_match('/^cs_[a-zA-Z0-9_]+$/', $sessionId)) {
            $page->getLayout()->getBlock('etechflow.mm.license.activated')
        ...
        // Per gotcha A: never rtrim($url, '/license/validate') — use str_replace.
        ...
        $this->configWriter->save('etechflow_megamenu/license/issued_domain',      $domain);
        $this->configWriter->save('etechflow_megamenu/license/issued_plan',        $plan);
        $this->configWriter->save('etechflow_megamenu/license/stripe_session_id',  $sessionId);
        $this->configWriter->save('etechflow_megamenu/license/revoked',            '0');
        $this->configWriter->save('etechflow_megamenu/license/ip_blocked',         '0');
    ...
    {
        $api = trim((string) $this->scopeConfig->getValue('etechflow_megamenu/license/portal_api_url'));
        if ($api !== '') return $api;
        return trim((string) $this->scopeConfig->getValue('etechflow_megamenu/license/portal_url'));
    }
